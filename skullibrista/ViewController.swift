...
//  skullibrista
//
//  Created by Guilherme Santos on 02/07/19.
...
class ViewController: UIViewController {
    ...
    func start() {
        ...
        if motionManager.isDeviceMotionAvailable {
            motionManager.startDeviceMotionUpdates(to: OperationQueue.main, withHandler: { (data, erro) in
                if erro == nil {
                    if let data = data {
    ...
    func checkGameOver() {
        ...
        if difference > 0.25 {
            ...
            lbTimePlayed.text = "Você jogou durante \(secondsPlayed) segundo\(secondsPlayed > 1 ? "s" : "")"
        }
    }
